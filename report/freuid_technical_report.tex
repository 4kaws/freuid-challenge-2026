...
\textbf{Code:} \url{https://github.com/4kaws/freuid-challenge-2026} (tag \texttt{v1.0-final})\\
...
\begin{itemize}[noitemsep]
  \item \textbf{Pick 1} \texttt{VARIANT=ens2}: rank-mean of EfficientNetV2-S (round 5)
        and EfficientNetV2-M (round 5), grid $3{\times}4$ (12 crops).
  \item \textbf{Pick 2} \texttt{VARIANT=hedge}: adds the round-1 pseudo-label model and
        the no-pseudo-label base model at half weight, grid $3{\times}4$ ---
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    \toprule
    Pick & Command & \texttt{sha256(submission.csv)}\footnotemark \\
    \midrule
    1 (\texttt{ens2})  & \texttt{docker run --network none ... -e VARIANT=ens2}  & \texttt{8647c6cc\,\ldots\,adfdab7b} \\
    2 (\texttt{hedge}) & \texttt{docker run --network none ... -e VARIANT=hedge} & \texttt{f54325e1\,\ldots\,c5e6d9fb} \\
    \bottomrule
  \end{tabular}
  ...
  {\scriptsize
  \begin{tabular}{ll}
    Pick 1: & \texttt{8647c6cc440dffeea1ce37d432ff929c43a29edfbd16f7eb79a6ff46adfdab7b} \\
    Pick 2: & \texttt{f54325e18a47bdd549f8155404f6de626fb50a6469f1ad3747d69566c5e6d9fb} \\
  \end{tabular}}
\end{table}
\footnotetext{Checksums of the submitted CSV files as produced by this pipeline
(per-model top-2-of-12-crop logits, joint rank averaging over all 142{,}818 test
images). Bit-level equality across GPU models is subject to floating-point
non-associativity; rank agreement is expected to be near-perfect (see
Section~\ref{sec:repro}, Randomness).}
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
    ...
    Final Pick 1 (\texttt{ens2}), $3{\times}4$ grid  & 0.00265 & Selected final submission \\
    Final Pick 2 (\texttt{hedge}), $3{\times}4$ grid & 0.01331 & OOD-robust variant \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
  \item \textbf{Repository:} \url{https://github.com/4kaws/freuid-challenge-2026},
        tag \texttt{v1.0-final} (MIT license); the exact commit SHA is quoted in the
        team's reply on the official submission thread.
  ...
  \item \textbf{Runtime:} measured throughput on an NVIDIA L4 (fp16, $512^2$ crops:
        160\,crops/s for V2-S, 123\,crops/s for V2-M) gives 6.9\,h (\texttt{ens2}) and
        12.8\,h (\texttt{hedge}) wall time for the full 142{,}818-image test set
        ($3{\times}4$ grid). On the A100 40GB verification target ($\geq 2.3\times$ L4
        throughput on these backbones) this extrapolates to $\approx$3.0\,h
        (\texttt{ens2}) and $\approx$5.5\,h (\texttt{hedge}), inside the 6-hour budget;
        the $3{\times}4$ grid was chosen over denser grids specifically to respect this
        budget. CPU fallback exists but is not practical at scale.
